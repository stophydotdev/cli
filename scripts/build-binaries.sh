...
TARGETS=(
  "bun-darwin-arm64:stophy-darwin-arm64"
  "bun-darwin-x64:stophy-darwin-x64"
  "bun-linux-x64:stophy-linux-x64"
  "bun-linux-arm64:stophy-linux-arm64"
  "bun-windows-x64:stophy-windows-x64.exe"
)
...
cd "$OUT_DIR"
shasum -a 256 stophy-* > checksums.txt 2>/dev/null || sha256sum stophy-* > checksums.txt
...
if command -v nfpm &>/dev/null; then
  ...
  for arch_pair in "x64:amd64" "arm64:arm64"; do
    bun_arch="${arch_pair%%:*}"
    nfpm_arch="${arch_pair##*:}"
    binary="$OUT_DIR/stophy-linux-$bun_arch"
